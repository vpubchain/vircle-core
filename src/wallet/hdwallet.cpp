...
bool CHDWallet::ProcessStakingSettings(std::string &sError)
{
    ...
    nStakeCombineThreshold = 100 * COIN;     // for benyuan
    nStakeSplitThreshold = 200 * COIN;      // for benyuan
    nMaxStakeCombine = 3;
    ...
    int nChangePosInOut = -1;
    {
        ...
        for (;;) {
            ...
            for (const auto &coin : setCoins) {
                ...
                if (!ProduceSignature(*this, DUMMY_SIGNATURE_CREATOR_VIRCLE, scriptPubKey, sigdata)) {
...
bool CHDWallet::CreateCoinStake(unsigned int nBits, int64_t nTime, int nBlockHeight, int64_t nFees, CMutableTransaction &txNew, CKey &key)
{
    ...
    if (!pDevFundSettings || pDevFundSettings->nMinDevStakePercent <= 0) {
        ...
        if (nBlockHeight % pDevFundSettings->nDevOutputPeriod == 0) {
            ...
            for (auto x: vData) {   std::cout << "vData-1=" << x; } std::count<< std::endl;  //2020-3-10
            for (auto x: vCfwd) {   std::cout << "vCfwd=" << x; }   std::count<< std::endl;  //2020-3-10
            vData.insert(vData.end(), vCfwd.begin(), vCfwd.end());
            for (auto x: vData) {   std::cout << "vData-2=" << x; } std::count<< std::endl;  //2020-3-10
...
bool IsVircleWallet(const CKeyStore *win)
{
