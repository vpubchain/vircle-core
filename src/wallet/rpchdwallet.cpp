...
static std::string SendHelp(CHDWallet *pwallet, OutputTypes typeIn, OutputTypes typeOut)
{
    ...
    rv += std::string(" vcl in a") + (typeOut == OUTPUT_RINGCT || typeOut == OUTPUT_CT ? " blinded" : "") + " payment to a given address"
        + (typeOut == OUTPUT_CT ? " in anon vcl": "") + ".\n";
    ...
    rv +=   "\nArguments:\n"
            ...
            "                            This is not vcl of the transaction, just kept in your wallet.\n"
            "4. \"comment_to\"  (string, optional) A comment to store the name of the person or organization \n"
            "                            to which you're sending the transaction. This is not vcl of the \n"
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 9)
        throw std::runtime_error(
            RPCHelpMan{"sendtypeto",
                "\nSend vcl to multiple outputs." +
                HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"typein", RPCArg::Type::STR, RPCArg::Optional::NO, "vcl/blind/anon"},
                    {"typeout", RPCArg::Type::STR, RPCArg::Optional::NO, "vcl/blind/anon"},
            ...
            "\"txid\"              (string) The transaction id.\n"
                ...
                RPCExamples{
            HelpExampleCli("sendtypeto", "anon vcl \"[{\\\"address\\\":\\\"PbpVcjgYatnkKgveaeqhkeQBFwjqR7jKBR\\\",\\\"amount\\\":0.1}]\"")
                },
            }.ToString());
