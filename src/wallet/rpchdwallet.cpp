...
static int ExtractBip32InfoV(const std::vector<uint8_t> &vchKey, UniValue &keyInfo, std::string &sError)
{
    ...
    if (memcmp(&vchKey[0], &Params().Base58Prefix(CChainParams::EXT_SECRET_KEY)[0], 4) == 0) {
        keyInfo.pushKV("type", "Vircle extended secret key");
    } else
...
static int ExtractBip32InfoP(const std::vector<uint8_t> &vchKey, UniValue &keyInfo, std::string &sError)
{
    ...
    if (memcmp(&vchKey[0], &Params().Base58Prefix(CChainParams::EXT_PUBLIC_KEY)[0], 4) == 0) {
        keyInfo.pushKV("type", "Vircle extended public key");
    } else
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            RPCHelpMan{"extkeyaltversion",
                ...
                "If the provided ext_key has a Bitcoin prefix the output will be encoded with a Vircle prefix.\n"
                "If the provided ext_key has a Vircle prefix the output will be encoded with a Bitcoin prefix.\n",
                {
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            RPCHelpMan{"getnewextaddress",
                "\nReturns a new Vircle ext address for receiving payments." +
                HelpRequiringPassphrase(pwallet) + "\n",
                {
...
static UniValue manageaddressbook(const JSONRPCRequest &request)
{
    ...
    if (!address.IsValid()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, _("Invalid Vircle address."));
    }
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValidStealthAddress())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Vircle stealth address: ")+input.get_str());
    ...
    if (request.params[0].isArray()) {
        ...
        for (size_t k = 0; k < outputs.size(); ++k) {
            ...
            if (typeOut == OUTPUT_RINGCT
                ...
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Vircle stealth address");
            }
...
static std::string SendHelp(CHDWallet *pwallet, OutputTypes typeIn, OutputTypes typeOut)
{
    ...
    rv += std::string(" tbc in a") + (typeOut == OUTPUT_RINGCT || typeOut == OUTPUT_CT ? " blinded" : "") + " payment to a given address"
        + (typeOut == OUTPUT_CT ? " in anon tbc": "") + ".\n";
    ...
    rv +=   "\nArguments:\n"
            ...
            "                            This is not tbc of the transaction, just kept in your wallet.\n"
            "4. \"comment_to\"  (string, optional) A comment to store the name of the person or organization \n"
            "                            to which you're sending the transaction. This is not tbc of the \n"
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 9)
        throw std::runtime_error(
            RPCHelpMan{"sendtypeto",
                "\nSend tbc to multiple outputs." +
                HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"typein", RPCArg::Type::STR, RPCArg::Optional::NO, "tbc/blind/anon"},
                    {"typeout", RPCArg::Type::STR, RPCArg::Optional::NO, "tbc/blind/anon"},
            ...
            "\"txid\"              (string) The transaction id.\n"
                ...
                RPCExamples{
            HelpExampleCli("sendtypeto", "anon tbc \"[{\\\"address\\\":\\\"PbpVcjgYatnkKgveaeqhkeQBFwjqR7jKBR\\\",\\\"amount\\\":0.1}]\"")
                },
            }.ToString());
...
static UniValue createrawparttransaction(const JSONRPCRequest& request)
{
    ...
    rawTx.nVersion = VIRCLE_TXN_VERSION;
...
static UniValue fundrawtransactionfrom(const JSONRPCRequest& request)
{
    ...
    tx.nVersion = VIRCLE_TXN_VERSION;
